...
export class NavigatorModel {
    ...
    phases(runId) {
        ...
        const seenPhases = new Set(order); // seeded: order starts from snap.phases
                    ...
                    case "save": {
                        ...
                        if (!run?.script)
                            ...
                            try {
                                ...
                                const registered = registerSavedWorkflow(pi, opts.getCwd ?? (() => opts.cwd ?? process.cwd()), saved, opts.getManager ?? (() => manager), () => (opts.getStorage?.() ?? opts.storage)?.load(saved.name) != null, () => (opts.getStorage?.() ?? opts.storage)?.load(saved.name));
                                ui.notify(registered.ok ? `Saved /${name}` : `Saved /${name}, but ${registered.message}`, registered.ok ? "info" : "error");
                            }
                            catch (error) {
