 ...
 */
export function describeOffendingPayload(text, max = 200) {
...
export async function resolveStructuredOutput(session, capture, schema, options, lastText) {
    ...
    throw new WorkflowError(`Subagent did not produce valid structured_output after repair attempts — model: ${modelNote}${changeNote}; offending payload: ${payload}`, WorkflowErrorCode.SCHEMA_NONCOMPLIANCE, {
        recoverable: false,
...
export class WorkflowAgent {
    ...
    noteRunModel(spec, trackForRun) {
        ...
        if (spec !== this.runDefaultModel)
            this.runModelChanged = true;
    }
    /**
     ...
     */
    assertRunModelAvailable(modelRegistry, label) {
        ...
        if (available.length === 0)
            return;
        if (available.some((model) => canonicalModelSpec(model) === spec))
            return;
        throw new WorkflowError(`Model "${spec}" is no longer enabled: it was the model this run started on, but it is gone from the available-models registry (a settings, auth, or provider change mid-run). Re-run with an available model (see /workflows-models).`, WorkflowErrorCode.MODEL_NOT_FOUND, { recoverable: false, agentLabel: label });
    }
    /**
    ...
    async runTurn(prompt, options = {}) {
        ...
        if (reliesOnRunDefault)
            this.assertRunModelAvailable(modelRegistry, options.label);
        ...
        try {
            ...
            throwIfProviderLimit(session.messages, options.label);
            if (options.schema) {
                const result = (await resolveStructuredOutput(session, capture, options.schema, {
