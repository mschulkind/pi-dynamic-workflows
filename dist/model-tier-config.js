...
export function getProjectModelTierConfigPath(cwd) {
...
function normalizeLoadOptions(configPathOrOptions) {
 ...
 */
export function loadModelTierConfig(configPathOrOptions) {
    ...
    if (!overlayRequested)
        return global;
    const projectPath = options.projectConfigPath ?? (options.cwd ? getProjectModelTierConfigPath(options.cwd) : undefined);
    if (!projectPath)
        return global;
